    ...
    <div className='rounded-xl border border-border/60 bg-card p-4 font-sans ambient-glow sm:p-5'>
      <div className='mb-4'>
        ...
        <div className='mt-3 flex flex-wrap items-center gap-2'>
          ...
          {inventoryItems.some((i) => i.status === 'RENTED') && (
            <div className='flex items-center gap-1.5 rounded-full bg-blue-100 px-3 py-1 text-xs font-semibold text-blue-700 dark:bg-blue-900/40 dark:text-blue-400'>
              {inventoryItems.filter((i) => i.status === 'RENTED').length} đang
              cho thuê
            </div>
          )}
      ...
      <div className='space-y-2'>
        {visibleItems.map((item) => {
          ...
          return (
            <div
              ...
              className='flex flex-col gap-2 rounded-lg border border-border bg-muted/30 p-3 sm:flex-row sm:items-center sm:justify-between sm:gap-3'
            >
        ...
        {!expanded && inventoryItems.length > 0 && (
