...
import { CalendarDays, CreditCard, Package, RotateCcw, Wallet } from 'lucide-react';
import {
  Dialog,
  DialogContent,
  DialogHeader,
        ...
        <div className="space-y-6 font-sans">
          ...
          <RadioGroup value={selectedOption} onValueChange={setSelectedOption}>
            <div className="flex items-center space-x-2">
              ...
              <Label htmlFor="default">Phường Bến Nghé, Quận 1, Hồ Chí Minh</Label>
            </div>
...
const processSteps = [
  ...
  {
    step: 2,
    icon: <CreditCard className="w-5 h-5 text-violet-500 dark:text-violet-400" />,
  ...
  {
    step: 4,
    icon: <RotateCcw className="w-5 h-5 text-orange-500 dark:text-orange-400" />,
        ...
        {processSteps.map((item, index) => (
          <li key={item.step} className="relative flex gap-3 pb-5 last:pb-0">
            {index < processSteps.length - 1 && (
              <span className="absolute top-9 bottom-0 left-[15px] w-px bg-border" aria-hidden />
            )}
