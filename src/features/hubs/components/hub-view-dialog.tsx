...
const INVENTORY_STATUS_CLASSES: Record<
  InventoryItemResponse["status"],
  string
> = {
  AVAILABLE:
...
function ProductInventoryGroupRow({
  ...
  const inventoryTotal =
    inventoryQuery.data?.meta.totalElements ?? inventoryRows.length;
      ...
      <div className="flex items-center gap-3 p-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs text-text-sub">
            {product.brand ?? "-"} ·{" "}
            {product.dailyPrice.toLocaleString("vi-VN")}₫/ngày
          </p>
      ...
      {expanded && (
        <div className="border-t border-gray-100 dark:border-white/8 px-3 pb-3 pt-2">
          {inventoryQuery.isLoading ? (
            ...
            <div className="py-4 text-xs text-text-sub">
              Không có serial nào với bộ lọc hiện tại.
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 dark:bg-white/4 text-left">
                    <th className="px-3 py-2 text-xs font-medium text-text-sub">
                      Serial
                    </th>
                    <th className="px-3 py-2 text-xs font-medium text-text-sub">
                      Màu
                    ...
                    <th className="hidden sm:table-cell px-3 py-2 text-xs font-medium text-text-sub">
                      Hub
                    </th>
                  </tr>
                </thead>
...
export function HubViewDialog({ hub, onClose }: HubViewDialogProps) {
  ...
  const assignedProductIds = useMemo(
    () =>
      new Set(
        (assignedProductsQuery.data?.content ?? []).map(
                  ...
                  <div className="mt-2 space-y-2">
                    ...
                    {productPickerQuery.isError && (
                      <p className="text-xs text-red-500">
                        Không thể tải danh sách sản phẩm để gán.
                      </p>
                    )}
                      ...
                      pickerProducts.length > 0 && (
                        <div className="space-y-1">
                          {pickerProducts.map((product) => {
                            const alreadyAssigned = assignedProductIds.has(
                              product.productId,
                            );
                            const isAssigningCurrent =
                              assignMutation.isPending &&
                              assignMutation.variables?.payload
                                .productIds?.[0] === product.productId;
                            ...
                            return (
                              ...
                              >
                                ...
                                <button
                                  type="button"
                                  disabled={
                                    alreadyAssigned || isAssigningCurrent
                                  }
                  ...
                  <>
                      ...
                      {hubProducts.map((product) => (
                        <ProductInventoryGroupRow
                          ...
                          isUnassigning={
                            ...
                            unassignMutation.variables?.payload
                              .productIds?.[0] === product.productId
                          }
                        />
                      ))}
                    ...
                    {hubProductsMeta && hubProductsMeta.totalPages > 1 && (
                      <div className="mt-3 flex items-center justify-between">
                        <span className="text-xs text-text-sub">
                          Trang {hubProductsMeta.currentPage}/
                          {hubProductsMeta.totalPages}
                        </span>
                        <div className="flex gap-1.5">
                          ...
                          <button
                            ...
                            onClick={() =>
                              setProductPage((p) =>
                                Math.min(
                                  (hubProductsMeta?.totalPages ?? 1) - 1,
                                  p + 1,
                                ),
                              )
                            }
                            disabled={
                              productPage >= hubProductsMeta.totalPages - 1
                            }
                            className="h-7 px-2.5 rounded-lg border border-gray-200 dark:border-white/12 text-xs font-medium text-text-main hover:bg-gray-50 dark:hover:bg-white/6 disabled:opacity-40 disabled:cursor-not-allowed transition"
                          >
                            Sau
