...
export async function generateContractPdf(
  ...
  row(
    "Phương thức TT:",
    input.paymentMethod === "e_wallet" ? "Ví điện tử" : "Chuyển khoản NH",
  );
  ...
  for (const line of input.lines) {
    ...
    page.drawText(line.durationLabel, {
      x: colX[1],
      y,
      size: 8,
      font: regular,
    ...
    const amount =
      (line.rentalAfterVoucher + line.depositTotal).toLocaleString("vi-VN") +
      "₫";
    page.drawText(amount, { x: colX[3], y, size: 8, font: bold, color: DARK });
    y -= 16;

    if (line.variantLabel) {
      page.drawText(`  ${line.variantLabel}`, {
        x: colX[0],
        y,
        size: 7,
        font: regular,
  ...
  totalRow(
    "Tiền thuê (trước giảm):",
    input.totals.rentalSubtotal.toLocaleString("vi-VN") + "₫",
  );
  if (input.totals.totalVoucherDiscount > 0) {
    totalRow(
      "Giảm voucher:",
      "−" + input.totals.totalVoucherDiscount.toLocaleString("vi-VN") + "₫",
    );
  }
  totalRow(
    "Tiền cọc:",
    input.totals.totalDeposit.toLocaleString("vi-VN") + "₫",
  );
  drawLine(page, y + 8, TEAL);
  y -= 4;
  totalRow(
    "TỔNG THANH TOÁN:",
    input.totals.grandTotal.toLocaleString("vi-VN") + "₫",
    true,
  );
  ...
  page.drawText("ĐIỀU KHOẢN & LƯU Ý", {
    x: MARGIN,
    y,
    size: 9,
    font: bold,
  ...
  page.drawText(
    "Tài liệu được tạo tự động bởi hệ thống Swiftera - swiftera.vn",
    {
      x: MARGIN,
      y: 10,
      size: 7,
